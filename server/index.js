...
//middlewares - functions that have access to the req, and the res object, they can perform any task specified before the output is sent to the client.
...
const connectOptions = {
  ...
  socketTimeoutMs: 30000, //time before socket timeout due to inactivity, umberuseful to avoid hanging connections
  retryWrites: true, // enables automatic retry of some write operationss like insert or update a document
  retryReads: true, //enables automatic retry of read operations
  maxPoolSize: 100, //maximum num of connections in mongodb connection pool, it helps to manage concurrent requests
  minPoolSize: 1, //minimun num of connections maintained by mongodb pool
};
...
const connectToDb = async () => {
  try {
    ...
    const gracefulShutdown = async () => {
      ...
      process.exit(0); //exit thenode process
    };
    process.on("SIGINT", gracefulShutdown); // signal interuption CTRL + C
    process.on("SIGTERM", gracefulShutdown); //signal term
    return conn;
  } catch (error) {
    console.log("MongoDb connection failing ", error.message);
...
const startServer = async () => {
  try {
    ...
    process.on("unhandledRejection", () => {
      console.error("UNHANDLED REJECTIONS Shutting down...");
      console.error(err.name, err.message);
      //close server gracefully
      server.close(() => {
        console.log("process terminated due to unhandles rejection");
      });
    });
