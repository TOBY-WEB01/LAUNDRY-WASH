...
export const dashboardStats = async (req, res, next) => {
  ...
  try {
    ...
    const paymentTotal = paymentData.reduce((acc, cv) => acc + cv.amount, 0);
...
export const getAllUsers = async (req, res, next) => {
  ...
  try {
    ...
    //user stats
    const now = new Date();
    const startOfCurrentMonthn = new Date(now.getFullYear(), now.getMonth(), 1);
    ...
    if (!booking.isPaid) {
      return next(
        responseHandler.errorResponse(
          "Pls update payment status before completing delivery",
          400
        )
      );
    }
