...
export default function Paystack({
  ...
  const hasOpenedRef = useRef(null);
  ...
  const handlePaystackCloseAction = useCallback(() => {
    ...
    //Reset Paystack component when user closes
    if (onClose) onClose();
  }, [onClose]);
  ...
  useEffect(() => {
    ...
    //Configure paystack and open paystack checkout
    const config = {
      email: user?.email,
      amount: total * 100, //Amount is in the country's lowest currency. Eg kobo, so 2000 kobo = N200
